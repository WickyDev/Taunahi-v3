package net.taunahi_v3.ezskyblock.feature.impl;
...
import net.taunahi.ezskyblockscripts.event.SpawnParticleEvent;
...
import net.taunahi.ezskyblockscripts.util.helper.*;
...
public class PestsDestroyer implements IFeature {
    ...
    public boolean isToggled() {
        return TaunahiConfig.enablePestsDestroyer;
    }
    ...
    public boolean canEnableMacro(boolean manually) {
        ...
        if (totalPests < TaunahiConfig.startKillingPestsAt && !manually) return false;
        if (!manually && TaunahiConfig.pausePestsDestroyerDuringJacobsContest && GameStateHandler.getInstance().inJacobContest()) {
    ...
    public void onTickExecute(TickEvent.ClientTickEvent event) {
        ...
        switch (state) {
            ...
            case OPEN_DESK:
                ...
                delayClock.schedule((long) (TaunahiConfig.pestAdditionalGUIDelay + 500 + Math.random() * 500));
                break;
            case OPEN_PLOTS:
                String chestName = InventoryUtils.getInventoryName();
                if (mc.currentScreen == null) {
                    state = States.OPEN_DESK;
                    delayClock.schedule((long) (TaunahiConfig.pestAdditionalGUIDelay + 300 + Math.random() * 300));
                    break;
                }
            ...
            case GET_LOCATION:
                ...
                getLocationTries++;
                if (!stuckClock.isScheduled())
                    stuckClock.schedule(1_000 * 60 * TaunahiConfig.pestsKillerStuckTime);
                delayClock.schedule(300);
                break;
            ...
            case FLY_TO_PEST:
                ...
                if (pestsLocations.isEmpty()) {
                    ...
                    KeyBindUtils.holdThese(
                            ...
                            TaunahiConfig.sprintWhileFlying ? mc.gameSettings.keyBindSprint : null,
            ...
            case KILL_PEST:
                ...
                if (TaunahiConfig.pestsKillerTicksOfNotSeeingPestWhileAttacking > 0
                ...
                if (cantReachPest >= TaunahiConfig.pestsKillerTicksOfNotSeeingPestWhileAttacking) {
                ...
                if (distance <= 2.5) {
                    ...
                    if (!RotationHandler.getInstance().isRotating()) {
                        ...
                        RotationHandler.getInstance().easeTo(new RotationConfiguration(
                                ...
                                TaunahiConfig.getRandomPestsKillerRotationTimeSmallDistance(),
                                null
                    ...
                    if (distance <= 10 || distanceXZ <= 2) {
                        ...
                        if (!RotationHandler.getInstance().isRotating()) {
                            ...
                            RotationHandler.getInstance().easeTo(new RotationConfiguration(
                                    new Target(entity),
                                    TaunahiConfig.getRandomPestsKillerRotationTimeMediumDistance(),
                                    null
                        ...
                        if (!GameStateHandler.getInstance().isLeftWalkable() && GameStateHandler.getInstance().isRightWalkable()) {
                            KeyBindUtils.holdThese(objects2 ? mc.gameSettings.keyBindJump : null, distanceXZ > 2 && yawDifference < 90 ? mc.gameSettings.keyBindForward : null, mc.gameSettings.keyBindRight, distanceXZ > 10 && yawDifference < 30 ? TaunahiConfig.sprintWhileFlying ? mc.gameSettings.keyBindSprint : null : null);
                        } else if (GameStateHandler.getInstance().isLeftWalkable() && !GameStateHandler.getInstance().isRightWalkable()) {
                            KeyBindUtils.holdThese(objects2 ? mc.gameSettings.keyBindJump : null, distanceXZ > 2 && yawDifference < 90 ? mc.gameSettings.keyBindForward : null, mc.gameSettings.keyBindLeft, distanceXZ > 10 && yawDifference < 30 ? TaunahiConfig.sprintWhileFlying ? mc.gameSettings.keyBindSprint : null : null);
                        } else {
                            KeyBindUtils.holdThese(objects2 ? mc.gameSettings.keyBindJump : null, distanceXZ > 2 && yawDifference < 90 ? mc.gameSettings.keyBindForward : null, distanceXZ > 10 && yawDifference < 30 ? TaunahiConfig.sprintWhileFlying ? mc.gameSettings.keyBindSprint : null : null);
                        }
    ...
    private boolean getVacuum(ItemStack currentItem2) {
        if (currentItem2 == null || !currentItem2.getDisplayName().contains("Vacuum")) {
            ...
            if (vacuum == -1) {
                ...
                state = States.GO_BACK;
                TaunahiConfig.enablePestsDestroyer = false;
                finishMacro();
                return true;
            }
    ...
    private void manipulateHeight(Entity entity, double distance, double distanceWithoutY, float yawDifference) {
        ...
        if (objectsInFrontOfPlayer() || entity.posY + entity.getEyeHeight() + 1 - mc.thePlayer.posY >= 2) {
            ...
            if (TaunahiConfig.sprintWhileFlying) {
                keyBindings.add(mc.gameSettings.keyBindSprint);
            }
            KeyBindUtils.holdThese(keyBindings.toArray(new KeyBinding[0]));
    ...
    public void onRender(RenderWorldLastEvent event) {
        ...
        for (Entity entity : pests) {
            ...
            if (TaunahiConfig.pestsESP) {
                Color color = TaunahiConfig.pestsESPColor.toJavaColor();
            ...
            if (TaunahiConfig.pestsTracers) {
                RenderUtils.drawTracer(new Vec3(entity.posX, entity.posY + entity.getEyeHeight(), entity.posZ), TaunahiConfig.pestsTracersColor.toJavaColor());
            }
        }

        if (!TaunahiConfig.highlightPlotWithPests) return;
        ...
        for (Map.Entry<Plot, Integer> plotNumber : pestsPlotMap.entrySet()) {
            ...
            boundingBox = boundingBox.offset(-d0, -d1, -d2);
            RenderUtils.drawBox(boundingBox, TaunahiConfig.plotHighlightColor.toJavaColor());
            ...
            if (line.contains("ൠ")) {
                ...
                try {
                    ...
                    if (pestsAmount != totalPests) {
                        ...
                        if (!isRunning() && totalPests >= TaunahiConfig.startKillingPestsAt) {
                            if (TaunahiConfig.sendWebhookLogIfPestsDetectionNumberExceeded) {
                                LogUtils.webhookLog("[Pests Destroyer]\\nThere " + (totalPests > 1 ? "are" : "is") + " currently **" + totalPests + "** " + (totalPests > 1 ? "pests" : "pest") + " in the garden!", TaunahiConfig.pingEveryoneOnPestsDetectionNumberExceeded);
                            }
                            if (TaunahiConfig.sendNotificationIfPestsDetectionNumberExceeded) {
