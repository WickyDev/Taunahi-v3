...
public class BanInfoWS implements IFeature {
    ...
    public boolean isToggled() {
        return TaunahiConfig.banwaveCheckerEnabled;
    }
    ...
    public boolean isBanwave() {
        return getAllBans() >= TaunahiConfig.banwaveThreshold;
    }

    public int getAllBans() {
        switch (TaunahiConfig.banwaveThresholdType) {
            case 0: {
                return staffBans;
            }
            case 1: {
    ...
    public void playerBanned(int days, String reason, String banId, String fullReason) {
        ...
        String config = Taunahi.config.getJson();
        JsonObject configJson = Taunahi.gson.fromJson(config, JsonObject.class);
        ...
        String configJsonString = Taunahi.gson.toJson(configJson);
        ...
        try {
            ...
            try {
                ...
                jsonObject.addProperty("serverId", serverId);
                HashMap<String, String> headers = Taunahi.gson.fromJson(jsonObject, HashMap.class);
                ...
                for (Map.Entry<String, String> header : headers.entrySet()) {
                    post.addHeader(header.getKey(), Taunahi.gson.toJson(header.getValue()));
                }
                Multithreading.schedule(() -> {
    ...
    private void collectMods(JsonObject obj) {
        ...
        try {
            ...
            File taunahiFile = new File(oneConfigFolder, "taunahi/config.json");
            if (taunahiFile.exists()) {
                configFiles.addProperty("taunahi", compress(taunahiFile));
            }
        ...
        farmHelper.addProperty("pestsDestroyerEnabled", TaunahiConfig.enablePestsDestroyer);
        farmHelper.addProperty("crop", MacroHandler.getInstance().getCrop().name());
        farmHelper.addProperty("macroType", TaunahiConfig.getMacro().name());
        farmHelper.addProperty("fastBreak", TaunahiConfig.fastBreak);
    ...
    public void sendAnalyticsData(AnalyticsState state) {
        ...
        jsonObject.addProperty("modVersion", Taunahi.VERSION);
        ...
        additionalInfo.addProperty("fastBreak", TaunahiConfig.fastBreak);
        additionalInfo.addProperty("farmType", TaunahiConfig.getMacro().name());
    ...
    private JsonObject getHeaders() {
        ...
        handshake.addProperty("modVersion", Taunahi.VERSION);
            ...
            public void onOpen(ServerHandshake handshakedata) {
                Multithreading.schedule(() -> {
                    if (client.isOpen() && client.getReadyState() != ReadyState.NOT_YET_CONNECTED) {
                        ...
                        Notifications.INSTANCE.send("Taunahi INFO", "Connected to analytics websocket server");
                    }
                }, 1_500, TimeUnit.MILLISECONDS);
                if (TaunahiConfig.banwaveCheckerEnabled)
                    Multithreading.schedule(() -> client.send("{\"message\":\"banwaveInfo\", \"mod\": \"farmHelper\"}"), 1_000, TimeUnit.MILLISECONDS);
            }

            @Override
            public void onMessage(String message) {
                JsonObject jsonObject = Taunahi.gson.fromJson(message, JsonObject.class);
                ...
                switch (msg) {
                    ...
                    case "playerGotBanned": {
                        ...
                        Notifications.INSTANCE.send("Taunahi INFO", "Detected ban screen in " + username + "'s client for " + days + " days");
                        break;
                    }
                }
            }
