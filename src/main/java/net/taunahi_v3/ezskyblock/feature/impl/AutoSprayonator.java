...
import net.taunahi.ezskyblockscripts.config.TaunahiConfig.SPRAYONATOR_ITEM;
...
import net.taunahi.ezskyblockscripts.util.PlotUtils;
...
public class AutoSprayonator implements IFeature {
    ...
    @Getter
    private SPRAYONATOR_ITEM sprayItem = SPRAYONATOR_ITEM.values()[TaunahiConfig.sprayonatorType];
    @Getter
    ...
    public void start() {
        ...
        sprayItem = SPRAYONATOR_ITEM.values()[TaunahiConfig.sprayonatorType];
    ...
    public boolean isToggled() {
        return TaunahiConfig.autoSprayonatorEnable;
    }
    ...
    private int getChosenSprayonatorSlot() {
        return TaunahiConfig.autoSprayonatorSlot - 1;
    }
    ...
    private boolean shouldBuySprayItem() {
        return !TaunahiConfig.sprayonatorItemInventoryOnly && !hasSprayItem();
    }
    ...
    public void onGuiOpen(DrawScreenAfterEvent event) {
        ...
        switch (sprayState) {
            case CHECK_PLOTS:
                sprayonatorDelay.schedule(TaunahiConfig.autoSprayonatorAdditionalDelay + 1000 + randomDelay);
                switch (checkPlotState) {
                    case OPEN_DESK:
                        if (!guiName.equals("Desk")) {
                            ...
                            Multithreading.schedule(() -> {
                                mc.thePlayer.closeScreen();
                                currentGuiState = CURRENT_GUI_STATE.NONE;
                            }, TaunahiConfig.autoSprayonatorAdditionalDelay + 100 + (long) (Math.random() * 50), TimeUnit.MILLISECONDS);
                            return;
                        }
                    ...
                    case OPEN_PLOTS:
                        ...
                        Multithreading.schedule(() -> {
                            ...
                            checkPlotState = CHECK_PLOT_STATE.NONE;
                            currentGuiState = CURRENT_GUI_STATE.NONE;
                        }, TaunahiConfig.autoSprayonatorAdditionalDelay + 150 + (long) (Math.random() * 50), TimeUnit.MILLISECONDS);
                }
                break;
            case BAZAAR_PURCHASE:
                sprayonatorDelay.schedule(TaunahiConfig.autoSprayonatorAdditionalDelay + 2000 + randomDelay * 3);
                if (guiName.contains("Bazaar")) {
                    ...
                    Multithreading.schedule(() -> {
                        mc.thePlayer.closeScreen();
                        currentGuiState = CURRENT_GUI_STATE.NONE;
                    }, TaunahiConfig.autoSprayonatorAdditionalDelay + 400 + (long) (Math.random() * 50), TimeUnit.MILLISECONDS);
                    return;
                }
                ...
                switch (bazaarPurchaseState) {
                    case CLICK_ITEM:
                        ...
                        }, TaunahiConfig.autoSprayonatorAdditionalDelay + 300 + (long) (Math.random() * 50), TimeUnit.MILLISECONDS);
                        break;
                    case BUY_ITEM:
                        Multithreading.schedule(() -> {
                            ...
                            if (guiName.contains(bazaarItemName) || guiName.contains(bazaarItemName.toLowerCase())) {
                                ...
                                if (slot == null) {
                                    ...
                                    Multithreading.schedule(() -> {
                                        mc.thePlayer.closeScreen();
                                        currentGuiState = CURRENT_GUI_STATE.NONE;
                                    }, TaunahiConfig.autoSprayonatorAdditionalDelay + 250 + (long) (Math.random() * 50), TimeUnit.MILLISECONDS);
                                    return;
                                }
                                Multithreading.schedule(() -> {
                                    InventoryUtils.clickContainerSlot(slot.slotNumber, InventoryUtils.ClickType.LEFT, InventoryUtils.ClickMode.PICKUP);
                                    ...
                                    }, TaunahiConfig.autoSprayonatorAdditionalDelay/2 + 300 + (long) (Math.random() * 50), TimeUnit.MILLISECONDS);
                                }, TaunahiConfig.autoSprayonatorAdditionalDelay/2 + 300 + (long) (Math.random() * 50), TimeUnit.MILLISECONDS);
                                bazaarPurchaseState = BAZAAR_PURCHASE_STATE.NONE;
                            } else {
