package net.taunahi_v3.ezskyblock.macro;
...
public abstract class AbstractMacro {
    ...
    public void onTick() {
        ...
        if (analyticsClock.passed() && TaunahiConfig.sendAnalyticData) {
        ...
        if (rewarpState == RewarpState.TELEPORTED) {
            ...
            if (rotated && !LagDetector.getInstance().isLagging() && mc.thePlayer.onGround) {
                ...
                rewarpDelay.schedule(TaunahiConfig.getRandomTimeBetweenChangingRows());
                return;
            ...
            if (TaunahiConfig.rotateAfterWarped) {
            ...
            if (shouldRotateAfterWarp()) {
                ...
                if (TaunahiConfig.dontFixAfterWarping) {
                ...
                rotation.easeTo(new RotationConfiguration(
                        newRotation, TaunahiConfig.getRandomRotationTime() * (Math.abs(neededChange.getYaw()) > 90 ? 2 : 1), null
                ).easeOutBack(true));
        ...
        if (TaunahiConfig.autoSwitchTool) {
            TaunahiConfig.CropEnum crop = PlayerUtils.getCropBasedOnMouseOver();
            if (crop != TaunahiConfig.CropEnum.NONE && crop != MacroHandler.getInstance().getCrop()) {
        ...
        if (GameStateHandler.getInstance().canChangeDirection()) {
            KeyBindUtils.stopMovement(TaunahiConfig.holdLeftClickWhenChangingRow);
            GameStateHandler.getInstance().scheduleNotMoving();
            updateState();
        } else {
    ...
    public void onEnable() {
        ...
        TaunahiConfig.CropEnum crop = PlayerUtils.getFarmingCrop();
        LogUtils.sendDebug("Crop: " + crop);
        MacroHandler.getInstance().setCrop(crop);
        PlayerUtils.getTool();
        GameStateHandler.getInstance().scheduleRewarp();
        if (TaunahiConfig.customPitch) {
            setPitch(TaunahiConfig.customPitchLevel);
        }
        if (TaunahiConfig.customYaw) {
            setYaw(TaunahiConfig.customYawLevel);
        }
