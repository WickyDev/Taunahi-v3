package net.taunahi_v3.ezskyblock.gui;

import net.taunahi.ezskyblockscripts.config.TaunahiConfig;
import net.taunahi.ezskyblockscripts.feature.impl.Proxy;
...
public class ProxyManagerGUI extends GuiScreen {
    ...
    public void initGui() {
        ...
        enableButton = new GuiButton(0, width / 2 - 100, height / 2 - 100, 200, 20, "Enable proxy: " + (TaunahiConfig.proxyEnabled ? "§a" : "§c") + TaunahiConfig.proxyEnabled);

        typeButton = new GuiButton(1, width / 2 - 100, height / 2 - 75, 200, 20, "Type: " + TaunahiConfig.proxyType);

        hostField = new GuiTextField(2, fontRendererObj, width / 2 - 100, height / 2 - 50, 200, 20);
        hostField.setMaxStringLength(100);
        hostField.setText(TaunahiConfig.proxyAddress);

        usernameField = new GuiTextField(3, fontRendererObj, width / 2 - 100, height / 2, 200, 20);
        usernameField.setMaxStringLength(100);
        usernameField.setText(TaunahiConfig.proxyUsername);

        passwordField = new GuiTextField(4, fontRendererObj, width / 2 - 100, height / 2 + 25, 200, 20);
        passwordField.setMaxStringLength(100);
        passwordField.setText(TaunahiConfig.proxyPassword);
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == 0) {
            TaunahiConfig.proxyEnabled = !TaunahiConfig.proxyEnabled;
            enableButton.displayString = "Enable proxy: " + (TaunahiConfig.proxyEnabled ? "§a" : "§c") + TaunahiConfig.proxyEnabled;
        } else if (button.id == 1) {
            if (TaunahiConfig.proxyType == Proxy.ProxyType.SOCKS) {
                TaunahiConfig.proxyType = Proxy.ProxyType.HTTP;
            } else {
                TaunahiConfig.proxyType = Proxy.ProxyType.SOCKS;
            }
            typeButton.displayString = "Type: " + TaunahiConfig.proxyType;
        } else if (button.id == 5) {
            Proxy.getInstance().setProxy(TaunahiConfig.proxyEnabled, hostField.getText(), TaunahiConfig.proxyType, usernameField.getText(), passwordField.getText());
