...
import net.taunahi.ezskyblockscripts.util.MarkdownFormatter;
...
public class AutoUpdaterGUI extends GuiScreen {
    ...
    private static final String GITHUB_API_URL = "https://github.com/SpareContributor/Taunahi_v3/releases";
        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                if (latestRelease != null) {
                    ...
                    String message = "";
                    if (isPreRelease && !TaunahiConfig.autoUpdaterDownloadBetaVersions) {
                    ...
                    if (isOutdated) {
                        ...
                        System.out.println("Taunahi is up to date!");
                    }
                } else {
    ...
    private static boolean isOutdated() {
        ...
        Matcher currentVersionMatcher = versionPattern.matcher(Taunahi.VERSION);
    ...
    private static int compareSymVer(String latestVersion) {
        Matcher currentMatcher = versionPattern.matcher(Taunahi.VERSION);
    ...
    private static void deleteAndClose() {
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            File[] filesToDelete = new File(mc.mcDataDir + "/mods").listFiles(
                    (dir, name) -> name.toLowerCase().startsWith("taunahi") && !name.toLowerCase().contains("jda") && !name.toLowerCase().contains(latestVersion));
    ...
    private void drawOutdatedMessage() {
        ...
        this.drawCenteredString(mc.fontRendererObj, "Outdated version of Taunahi", (int) (this.width / 2f / scale),
    ...
    private void drawUpdateInProgressMessage() {
        ...
        this.drawCenteredString(mc.fontRendererObj, "Updating Taunahi... (" + downloadProgress + "%) Please wait",
    ...
    private void drawUpdateCompletedMessage() {
        float scale = 2;
        GL11.glScalef(scale, scale, 0.0F);
        this.drawCenteredString(mc.fontRendererObj, "Taunahi has been updated!", (int) (this.width / 2f / scale),
                    ...
                    executor.submit(() -> {
                        try {
                            ...
                            downloadFileWithProgress(
                                    downloadURL,
                                    new File(mc.mcDataDir + "/mods/Taunahi_v3-" + latestVersion + ".jar")
                            );
                            mc.addScheduledTask(() -> {
                                downloadBtn.enabled = false;
