package net.taunahi_v3.ezskyblock.remote;
...
public class DiscordBotHandler extends ListenerAdapter {
    ...
    public void connect() {
        ...
        try {
            ...
            Notifications.INSTANCE.send("Taunahi", "Connected to the Discord Bot!");
            LogUtils.sendSuccess("Connected to the Discord Bot!");
        } catch (InvalidTokenException | ErrorResponseException e) {
            e.printStackTrace();
            Notifications.INSTANCE.send("Taunahi", "Failed to connect to the Discord Bot! Check your token. Disabling remote control and removing the token...");
            ...
            if (e.getMessage().contains("im-getting-closecode4014-disallowed-intents")) {
                Notifications.INSTANCE.send("Taunahi", "You need to enable Intents in the Discord Developer Portal! Disabling remote control...");
                ...
                Notifications.INSTANCE.send("Taunahi", "Discord Bot is already connected, connecting as a client...");
                LogUtils.sendWarning("Discord Bot is already connected, connecting as a client...");
                WebsocketHandler.getInstance().setWebsocketState(WebsocketHandler.WebsocketState.CLIENT);
            }
        } catch (InterruptedException e) {
            Notifications.INSTANCE.send("Taunahi", "Unexpected error while connecting to the Discord Bot, disabling remote control...");
