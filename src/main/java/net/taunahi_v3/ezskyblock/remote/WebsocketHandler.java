    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        if (!Loader.isModLoaded("taunahijdadependency")) {
            if (TaunahiConfig.enableRemoteControl) {
                ...
                Notifications.INSTANCE.send("Taunahi", "Taunahi JDA Dependency is not installed, disabling remote control..");
            }
            return;
        }
        ...
        switch (websocketState) {
            ...
            case CLIENT: {
                if (websocketClient == null) {
                    try {
                        ...
                        Notifications.INSTANCE.send("Taunahi", "Connected to websocket server as a client!");
                    } catch (URISyntaxException | InterruptedException e) {
                        LogUtils.sendDebug("[Remote Control] Failed to connect to the websocket server!");
                        e.printStackTrace();
                    }
                    ...
                    if (reconnectAttempts > 5) {
                        ...
                        Notifications.INSTANCE.send("Taunahi", "Failed to connect to the websocket server, disabling remote control..");
            ...
            case SERVER: {
                if (websocketServer == null) {
                    ...
                    Notifications.INSTANCE.send("Taunahi", "Started websocket server on port " + TaunahiConfig.remoteControlPort);
