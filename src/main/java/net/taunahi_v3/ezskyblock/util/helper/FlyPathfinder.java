package net.taunahi_v3.ezskyblock.util.helper;
...
public class FlyPathfinder {
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        if (distance3d < 0.5 || (isPlayerUnderHalfBlock() && distance3d < 1)) {
            ...
            if (!pathBlocks.isEmpty()) {
                ...
                float yaw = (TaunahiConfig.flightLockRotationToMultipliersOf45Degrees ? AngleUtils.getClosest45(rotation.getRotation(target, true).getYaw()) : rotation.getRotation(target, true).getYaw());
                rotation.easeTo(
        ...
        KeyBindUtils.setKeyBindState(mc.gameSettings.keyBindRight, relativeDistanceX < -TaunahiConfig.flightAllowedOvershootThreshold);
        KeyBindUtils.setKeyBindState(mc.gameSettings.keyBindLeft, relativeDistanceX > TaunahiConfig.flightAllowedOvershootThreshold);
        KeyBindUtils.setKeyBindState(mc.gameSettings.keyBindForward, relativeDistanceZ > TaunahiConfig.flightAllowedOvershootThreshold);
        KeyBindUtils.setKeyBindState(mc.gameSettings.keyBindBack, relativeDistanceZ < -TaunahiConfig.flightAllowedOvershootThreshold);
    ...
    public boolean isStuck() {
        if (getPlayerSpeed() < 0.1 && !mc.thePlayer.onGround)
            ...
            stuckCounterWithoutMotion = 0;
        if (stuckCounterWithoutMotion > TaunahiConfig.flightMaxStuckTimeWithoutMotion) {
            stuckCounterWithoutMotion = 0;
            return true;
        }
        return false;
    ...
    public boolean isStuckWithMotion() {
        ...
        if (currentPlayerPos.distanceTo(lastPlayerPos) < TaunahiConfig.flightMaximumStuckDistanceThreshold)
            stuckCounterWithMotion++;
        else {
            stuckCounterWithMotion = 0;
            lastPlayerPos = currentPlayerPos;
        }
        if (stuckCounterWithMotion > TaunahiConfig.flightMaxStuckTimeWithMotion) {
            stuckCounterWithMotion = 0;
            return true;
        }
        return false;
    ...
    public boolean decelerationReached(double motion, double distance) {
        ...
        int offset = TaunahiConfig.flightDecelerationOffset;
